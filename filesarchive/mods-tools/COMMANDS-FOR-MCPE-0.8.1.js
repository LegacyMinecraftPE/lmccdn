 ...
 *
 * COMMANDS FOR MCPE 0.8.1
 * Made by ALEXCRAFT77 & 19FER97
 * Sprint script made by 19FER97, modified by ALEXCRAFT77 (Used with permission)
 * The entity command is based from the entity manager in MrARM's TMI. Thanks MrARM!
 *
 * © Copyright 2013 ALEXCRAFT77 & 19FER97
...
function procCmd(c) {
	...
	switch(command.toLowerCase()) {
		case 'commands': {
			clientMessage("[AC77_19FER97] Type /help");
			break;
		} case 'help': {
			switch(p[1]) {
				case 'ascend': {
					clientMessage("[AC77_19FER97] [HELP] Type /ascend to teleport to the platform above you.\nExample: /ascend");
					break;
				} case 'bomb': {
					clientMessage("[AC77_19FER97] [HELP] Type /bomb <on|off|detonate>. Use a Stick or Iron sword as the bomb setter, when Bomb Mode is on. WARNING: May be explosive!\n Example: /bomb on");
					break;
				} case 'bounce': {
					clientMessage("[AC77_19FER97] [HELP] Type /bounce <power> to get launched into the air\nExample: /bounce 2");
					break;
				} case 'coords': {
					clientMessage("[AC77_19FER97] [HELP] Type /coords to get the current coordinates at your feet.\nExample: /coords");
					break;
				} case 'delhome': {
					clientMessage("[AC77_19FER97] [HELP] Type /delhome to delete your current home coordinates.\nExample: /delhome");
					break;
				} case 'descend': {
					clientMessage("[AC77_19FER97] [HELP] Type /descend to teleport to the platform below you.\nExample: /descend");
					break;
				} case 'enderpearl': {
					clientMessage("[AC77_19FER97] [HELP] Type /enderpearl <on|off> while holding an egg in creative mode to activate/deactivate ender pearls.\nExample: /enderpearl on");
					break;
				} case 'entity': {
					clientMessage("[AC77_19FER97] [HELP] Type /entity <kill|remove|burn|explode> <MobName|all> to remove/burn/explode the specified entities.\nExample: /entity burn all");
				} case 'eval': {
					clientMessage("[AC77_19FER97] [HELP] Type /eval <code> to run a script in game!\nExample: /eval clientMessage(\"Hello world!\");");
					break;
				} case 'explode': {
					clientMessage("[AC77_19FER97] [HELP] Type /explode <radius> to blow up. WARNING: It may hurt/nExample: /explode 5");
					break;
				} case 'gamemode': {
					clientMessage("[AC77_19FER97] [HELP] Type /gamemode <survival|creative|0|1> to change your current gamemode. NOTE: Clears your survival inventory");
					break;
				} case 'give': {
					clientMessage("[AC77_19FER97] [HELP] Type /give <ID> <amount> to add any item to your inventory.\nExample: /give 57 64");
					break;
				} case 'heal': {
					clientMessage("[AC77_19FER97] [HELP] Type /heal or /heal <Half-hearts> to set your health to the specified amount.\nExample: /heal 20");
					break;
				} case 'hole': {
					clientMessage("[AC77_19FER97] [HELP] Type /hole to commit suicide. WARNING: USE WITH CAUTION!\n Example: /hole");
					break;
				} case 'home': {
					clientMessage("[AC77_19FER97] [HELP] Type /home to tp to your home (Use /sethome first)\nExample: /home");
					break;
				} case 'ignite': {
					clientMessage("[AC77_19FER97] [HELP] Type /ignite or /ignite <seconds> to set you on fire. WARNING: High chance of being burnt.\nExample: /ignite 3");
					break;
				} case 'instabreak': {
					clientMessage("[AC77_19FER97] [HELP] Type /instabreak <on|off> to turn InstaBreak on or off.\nExample: /instabreak on");
					break;
				} case 'launch': {
					clientMessage("[AC77_19FER9] [HELP] Type /launch <MobName|tnt> <rapid> to launch a mob or tnt in the direction you are facing!");
					break;
				} case 'kill': {
					clientMessage("[AC77_19FER97] [HELP] Type /kill to kill yourself");
					break;
				} case 'mc': {
					clientMessage("[AC77_19FER97] [HELP] Type /mc <on|off> to activate or deactivate the Magic Carpet.\nExample: /mc on");
					break;
				} case 'nuke': {
					clientMessage("[AC77_19FER97] [HELP] Type /nuke to cause a MAHOOSIVE explosion!\nExample: /nuke");
					break;
				} case 'panorama': {
					clientMessage("[AC77_19FER97] [HELP] Type /panorama <on|off> to activate or deactivate Panorama Mode.\nExample: /panorama on");
					break;
				} case 'pdoor': {
					clientMessage("[AC77_19FER97] [HELP] Type /pdoor <on|off|open>. Use a stick or Iron hoe to set the position of the door.\nExample: /pdoor on");
					break;
				} case 'rain': {
					clientMessage("[AVoiDED_19FER97] [HELP] Type /rain <MobName> to make it rain animals!\nExample: /rain chicken");
					break;
				} case 'refresh': {
					clientMessage("[AC77_19FER97] [HELP] Type /refresh to regain all items required for currently active commands.\nExample: /refresh");
					break;
				} case 'setitem': {
					clientMessage("[AC77_19FER97] [HELP] Type /setitem <ID> to set the specified item ID to your current held item.\nExample: /setitem 264");
					break;
				} case 'sethome': {
					clientMessage("[AC77_19FER97] [HELP] Type /sethome to set coordinates you can easily tp back to, using /home.\n Example: /sethome");
					break;
				} case 'spawn': {
					clientMessage("[AC77_19FER97] [HELP] Type /spawn <MobName> <Amount> to spawn the specified mob.\nExample: /spawn pig 5");
					break;
				} case 'spawntouch': {
					clientMessage("[AC77_19FER97] [HELP] Type /spawntouch <MobName|off> to spawn the specified mob when you tap a block.\nExample: /spawntouch chicken");
					break;
				} case 'sprint': {
					clientMessage("[AC77_19FER97] [HELP] Type /sprint <on|off> to activate or deactivate Sprint Mode.\nExample: /sprint on\nOriginal Sprint Script made by ALEXCRAFT77 & 19FER97.");
					break;
				} case 'summon': {
					clientMessage("[AC77_19FER97] [HELP] Type /summon <mob> <x> <y> <z> to spawn a mob at the specified coordinates.\nExample: /summon pig 147 84 123");
					break;
				} case 'surface': {
					clientMessage("[AC77_19FER97] [HELP] Type /surface to teleport to the surface above you.\nExample: /surface");
					break;
				} case 'time': {
					clientMessage("[AC77_19FER97] [HELP] Type /time or /time set <sunrise|day|sunset|night|value> to get the time, or set the time to the specified time (respectively). NOTE: Does not work properly.\nExample: /time set day");
					break;
				} case 'tp': {
					clientMessage("[AC77_19FER97] [HELP] Type /tp <x> <y> <z>, where x, y, and z are your desired coordinates.\n Example: /tp 128 70 128)");
					break;
				} case 'warp': {
					clientMessage("[AC77_19FER97] [HELP] Type /warp <on|off> to turn Warp Panels on or off.\nExample: /warp on");
					break;
				} case '1': {
		...
		if(Level.getGameMode() == 1) {
			...
			clientMessage("[AC77_19FER97] You need to be in creative mode!\n(Type /gamemode creative)");
		}
		break;
			...
			if(p[1] == 'on') {
				if(sprintMode == 1) {
					...
					sprintMode = 1;
					colourChat("Sprint Mode activated! MADE BY ALEXCRAFT77 & 19FER97.");
				}
			} if(p[1] == 'off') {
				if(sprintMode == 0) {
...
function colourChat(msg) {
	try {
		clientMessage(ChatColor.RED + "[AC77_19FER97] " + ChatColor.GREEN + msg);
	} catch(err) {
		clientMessage("[AC77_19FER97] " + msg);
	}
}
